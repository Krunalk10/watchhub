	...
	try {
		return await Promise.resolve().then(async () => {
			try {
				...
				console.log("Fetching watch with ID:", watchId);
				...
				const relatedWatches = await Promise.resolve(
					watchesData.watches.filter(
						(w) =>
							watch.relatedProducts && watch.relatedProducts.includes(w.id),
					),
				);

				console.log(
					"Found watch and",
					relatedWatches.length,
					"related watches",
				);
		...
		console.error(
			" Watch detail API error:",
			error instanceof Error ? error.message : String(error),
		);
		return NextResponse.json(
			{
				error: "Failed to fetch watch details",
				details:
